...
namespace Go_WinApp
{
	...
	public class GoBoard : System.Windows.Forms.Form
	{
		...
		int nSize;		                //棋盘大小
		const int nBoardMargin = 10;	//棋盘边框
		int nCoodStart = 4;
		const int	nBoardOffset = 20;
		int nEdgeLen = nBoardOffset + nBoardMargin;
		int nTotalGridWidth = 360 + 36;	//总宽度
		int nUnitGridWidth = 22;		//每个格大小
		int nSeq = 0;
		Rectangle rGrid;		    //矩形单元格
		StoneColor m_colorToPlay;   //下一步的棋子颜色 
		GoMove m_gmLastMove;	    //上一步移动
		Boolean bDrawMark;	        //是否已经画上棋子
		Boolean m_fAnyKill;	        //是否有气
		Spot [,] Grid;		        //所有
		Pen penGrid, penStoneW, penStoneB,penMarkW, penMarkB;
		Brush brStar, brBoard, brBlack, brWhite, m_brMark;
	
        // 主程序部分
        int nFFMove = 10;   //一次向前移动10步
        int nRewindMove = 10;  // 一次向后移动10步
		...
		{
			GoMove gm = gameTree.doPrev();	//向前遍历
            if (null != gm)
            {
                playPrev(gm);
            }
     ...
     textBox1.Clear();
		...
		*/
		...
		void drawLabel(Graphics g, int x, int y, short nLabel) 
		{
			...
			g.DrawString(strLabels[nLabel],	//写入字符
				this.Font, 
				brBlack, 
	...
	public class GoMove 
	{
		...
		int m_n;			
		String m_comment;	//评论
		MoveResult m_mr;	//移动结果

		ArrayList		m_alLabel; //所有标签
		ArrayList		m_alMark; //所有标记
	...
	*/
	...
	struct GameInfo 
	{
        ...
        public string unknown_ff;   //不知道颜色
		...
		public KeyValuePair(string k, string v)
		{
			...
			while (iBegin < v.Length && iBegin>=0)
			{
				...
				if (iEnd > 0)
					...
					strOneVal = v.Substring(iBegin);	//起始点
				alV.Add(strOneVal);
	...
	public class GoTree 
	{
		GameInfo _gi;		//游戏信息
		ArrayList _vars;		//变量 
		int _currVarId;		//当前玩家id
		int _currVarNum;
		GoVariation _currVar;		//当前游戏信息
		...
		Boolean parseFile(String goStr) 
		{
			...
			while (iEnd < goStr.Length) 
			{
				...
				if (iEnd < 0) //ZZ ZZZZ ";"
					iEnd = goStr.LastIndexOf(")", goStr.Length);		//如果遇到)
				if (iBeg >= 0 && iEnd > iBeg) 
				{
		...
		Boolean parseASection(String sec) 
		{
			...
			try 
			{
				...
				while (iKey > 0 && iValue > iKey)//循环遍历
				{
					...
					if (iKey > 0)
					{
						iValue = findEndofValueStr(sec); //找]
					}
				}
			}
			catch
